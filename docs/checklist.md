...
| 16 (일부) | 언론사 72개 목록 데이터 + 티커 데이터 | 2주차 그리드 작업 전 필요 |

---

## 2주차 목표 (섹션 6 → 15 + 섹션 16 나머지)
...
- [ ] 전체 캔버스 1280px 고정, 콘텐츠 영역 930px (좌우 여백 각 175px)
- [ ] 1280px 미만 뷰포트에서 콘텐츠 스케일 다운 처리
- [ ] 수직 위치 고정 — Header y:58 (h:29), Ticker y:127 (h:49), TabBar y:208 (h:24), Content y:256 (930×388)
- [ ] 체브론 위치 고정 — 콘텐츠 영역 외부, 좌 x:103 / 우 x:1153, y:430 (24×40)

---

## 3. 헤더 (Header)

- [ ] flex space-between 레이아웃, height 29
- [ ] 좌측 — 신문 아이콘 24×24 (stroke `#14212B`) + "뉴스스탠드" 텍스트 (display 24/700 `ink`)
- [ ] 우측 — 오늘 날짜를 "YYYY. MM. DD. 요일" 형식으로 동적 표시 (body 16/500 `sub`)
...
- [ ] flex space-between 레이아웃, height 24
- [ ] 좌측 — "전체 언론사" 탭, 활성 시 16/700 `ink` / 비활성 시 16/500 `mute`
- [ ] 좌측 — "내가 구독한 언론사" 탭, 동일 활성·비활성 스타일 규칙
- [ ] 구독 수 뱃지 — 20×20, `r-badge(10px)`, bg `#7890E7`, 숫자 12/500 `rgba(255,255,255,0.7)`, 구독 수 동적 반영
- [ ] 좌측 탭 클러스터 gap 24
- [ ] 우측 — 리스트 뷰 아이콘 24×24, 활성 시 `ink` / 비활성 시 `mute`
- [ ] 우측 — 그리드 뷰 아이콘 24×24, 활성 시 `ink` / 비활성 시 `mute`
- [ ] 우측 아이콘 클러스터 gap 8
...
- [ ] 동일 6×4 그리드 구조 유지
- [ ] 구독한 언론사 셀만 워드마크 표시, 나머지 셀은 `#FFFFFF` 빈 셀 유지
- [ ] 구독 수 ≤ 24: 1페이지, 초과 시 추가 페이지 (페이지당 최대 24개)
- [ ] 구독이 0개일 때 — 전체 셀 빈 상태
...
- [ ] 좌우 체브론 버튼 — 24×40, 윤곽선 스타일, stroke `#879298` 1.4px, 화살표 글리프
- [ ] 콘텐츠 영역 외부 고정 위치 — 좌 x:103 / 우 x:1153, y:430
- [ ] 첫 페이지: 좌측 체브론 `opacity: 0` (레이아웃 공간 유지, 시각만 제거)
- [ ] 마지막 페이지: 우측 체브론 `opacity: 0`
- [ ] `<button>` 태그, `aria-label="이전 페이지"` / `"다음 페이지"`, 비활성 시 `disabled` 속성
